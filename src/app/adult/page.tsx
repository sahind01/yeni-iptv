...
import { FiSearch, FiX, FiLock, FiPlay } from 'react-icons/fi';
...
export default function AdultPage() {
  ...
  const fetchChannels = async () => {
    try {
      setIsLoading(true); setError(''); setDebugInfo('');
      ...
      // Debug: ilk 3 kanalın URL'lerini göster
      if (parsed.length > 0) {
        const sample = parsed.slice(0, 3).map(c => `${c.name}: ${c.url.substring(0, 50)}...`).join('\n');
  ...
  return (
    <MainLayout>
      <div className="p-3 sm:p-4 lg:p-6">
        ...
        {hasMore && <div className="text-center mt-6"><button onClick={() => setShowCount(p => p + 20)} className="px-6 py-2.5 bg-[#1a1a1a] border border-gray-700/50 rounded-xl text-sm">Daha Fazla</button></div>}
      </div>
    </MainLayout>
  );
}
