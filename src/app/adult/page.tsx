...
export default function AdultPage() {
  ...
  // Sayfa yüklendiğinde PIN durumunu kontrol et
  useEffect(() => {
    checkPinStatus();
  }, [userId]);
  ...
  const checkPinStatus = async () => {
    ...
    if (savedPin) {
      // Kullanıcının PIN'i var
      setHasExistingPin(true);
    } else {
      // Kullanıcının PIN'i yok - oluşturması gerek
      setHasExistingPin(false);
    }
    setCheckingPin(false);
  };

  // PIN doğrulandığında kanalları yükle
  useEffect(() => {
    if (pinVerified) {
      fetchChannels();
    }
  ...
  const handlePinSubmit = async () => {
    ...
    if (!savedPin) {
      setPinError('PIN bulunamadı, lütfen PIN oluşturun');
      return;
    }

    if (CryptoUtils.verifyPin(pinInput, savedPin)) {
      setPinVerified(true);
  ...
  const handleCreatePin = async () => {
    ...
    setPinSuccess('PIN başarıyla oluşturuldu!');
    setHasExistingPin(true);
  ...
  const handleChangePin = async () => {
    if (newPin.length !== 4) {
      setPinError('Yeni PIN 4 haneli olmalıdır');
      return;
    }
    if (newPin !== confirmPin) {
      setPinError('PIN\'ler eşleşmiyor');
  ...
  if (!pinVerified && showPinScreen) {
    return (
      <MainLayout>
            ...
            {hasExistingPin === false ? (
              <>
                <p className="text-gray-400 text-sm mb-6">
                  Bu bölüme erişmek için 4 haneli bir PIN oluşturmalısınız.
                </p>
                <button
                  onClick={() => setShowCreatePin(true)}
                  className="w-full py-3 bg-blue-500 hover:bg-blue-600 rounded-xl text-sm font-medium transition-all"
                >
                  PIN Oluştur
                </button>
              </>
        ...
        {/* PIN Oluşturma / Değiştirme Modal */}
        <AnimatePresence>
          {(showCreatePin || showChangePin) && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
            ...
            >
              ...
              >
                <h3 className="text-lg font-semibold mb-4">
                  {showCreatePin ? 'PIN Oluştur' : 'PIN Değiştir'}
                </h3>
                ...
                {pinError && <p className="text-red-400 text-xs mt-3">{pinError}</p>}
                {pinSuccess && <p className="text-green-400 text-xs mt-3">{pinSuccess}</p>}
                ...
                <div className="flex space-x-2 mt-4">
                  <button
                    ...
                    className="flex-1 py-2.5 bg-gray-700 hover:bg-gray-600 rounded-xl text-sm"
                  >
                    İptal
                  </button>
                  <button
                    onClick={showCreatePin ? handleCreatePin : handleChangePin}
                    className="flex-1 py-2.5 bg-blue-500 hover:bg-blue-600 rounded-xl text-sm font-medium"
                  >
                    {showCreatePin ? 'Oluştur' : 'Kaydet'}
                  </button>
                </div>
              </motion.div>
            </motion.div>
    ...
    <MainLayout>
      <div className="p-3 sm:p-4 lg:p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h1 className="text-xl sm:text-2xl font-bold flex items-center gap-2">🔞 Adult</h1>
            <p className="text-xs sm:text-sm text-gray-400 mt-0.5">{channels.length} kanal</p>
          </div>
        ...
        <div className="relative mb-4">
          ...
          />
          {searchQuery && (
            <button onClick={() => setSearchQuery('')} className="absolute right-3 top-1/2 -translate-y-1/2 p-1 hover:bg-white/10 rounded-lg">
          ...
          <>
                ...
                {displayedChannels.map((channel) => (
                  ...
                  >
                    <div className="aspect-video bg-[#111] flex items-center justify-center p-3 relative">
                      {channel.logo ? (
                        <img src={channel.logo} alt={channel.name} className="max-w-full max-h-full object-contain" loading="lazy"
                          onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
                      ) : null}
                      <div className={`${channel.logo ? 'hidden' : ''} flex flex-col items-center`}>
                        <span className="text-2xl mb-1">🔞</span>
                        <span className="text-[10px] text-gray-500 text-center px-2 line-clamp-2">{channel.name}</span>
                      </div>
                      <div className="absolute top-2 right-2 px-1.5 py-0.5 bg-red-500/80 rounded text-[9px] font-medium">HD</div>
                    </div>
                    <div className="p-2 sm:p-3">
                      <h3 className="text-[11px] sm:text-xs font-medium text-white truncate">{channel.name}</h3>
                      <p className="text-[9px] sm:text-[10px] text-gray-500 mt-0.5">{channel.group}</p>
                    </div>
                  </motion.div>
                ))}
              </AnimatePresence>
            ...
            {hasMore && (
              <div className="text-center mt-6">
                <button onClick={loadMore} className="px-6 py-2.5 bg-[#1a1a1a] hover:bg-[#252525] border border-gray-700/50 rounded-xl text-sm text-gray-300 transition-colors">
