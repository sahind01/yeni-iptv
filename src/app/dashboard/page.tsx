...
import { FiShield, FiMaximize2, FiMinimize2, FiSmartphone, FiLock, FiStar, FiSend, FiCheck, FiMessageSquare } from 'react-icons/fi';
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-4">
          <div className="flex items-center justify-between mb-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[9px] bg-green-500/20 text-green-400 px-1.5 py-0.5 rounded-full">CANLI</span>
            </div>
            <button onClick={() => setIframeFull(!iframeFull)} className="p-1 hover:bg-white/10 rounded-lg text-gray-400 hover:text-white transition-all">
              {iframeFull ? <FiMinimize2 className="w-3 h-3" /> : <FiMaximize2 className="w-3 h-3" />}
            </button>
          </div>
          <div className={`rounded-xl border border-gray-700 overflow-hidden bg-[#111] transition-all duration-300 ${iframeFull ? 'h-[500px] sm:h-[600px]' : 'h-[250px] sm:h-[280px]'}`}>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => setFormType('istek')}
                      className={`flex-1 py-2.5 rounded-lg text-xs font-medium transition-all ${
                        formType === 'istek' ? 'bg-blue-500/20 border border-blue-500 text-blue-400' : 'bg-white/5 border border-gray-700 text-gray-400'
                      }`}>
                      💡 İstek
                    </button>
                    <button onClick={() => setFormType('sikayet')}
                      className={`flex-1 py-2.5 rounded-lg text-xs font-medium transition-all ${
                        formType === 'sikayet' ? 'bg-red-500/20 border border-red-500 text-red-400' : 'bg-white/5 border border-gray-700 text-gray-400'
                      }`}>
                      ⚠️ Şikayet
                    </button>
                  </div>
                  ...
                  <div>
                    ...
                    <input type="text" value={formNick} onChange={(e) => setFormNick(e.target.value)}
                      placeholder="Anonim" className="w-full bg-[#111] border border-gray-700 rounded-lg px-3 py-2 text-xs text-white placeholder-gray-500 focus:outline-none focus:border-blue-500" maxLength={30} />
                  </div>
                  <div>
