  ...
  return (
    <MainLayout>
      <div className="p-4">
        {/* BAŞLIK */}
        <div className="mb-4">
          <h1 className="text-lg font-bold flex items-center gap-2"><FiFilm className="text-blue-400" /> Filmler</h1>
          {!selectedCategory && <p className="text-xs text-gray-400 mt-0.5">{categories.length} kategori</p>}
        </div>

        {!selectedCategory ? (
          /* KATEGORİ LİSTESİ - MOBİL KARTLAR */
          <div className="grid grid-cols-2 gap-2">
            {categories.map((cat, index) => (
              ...
              >
                ...
                <span className="text-xs font-medium text-white text-center line-clamp-2">{cat.name}</span>
              </motion.button>
            ))}
          </div>
        ) : (
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mb-3">
              <button onClick={handleBackToCategories} className="p-2.5 bg-gray-800 hover:bg-gray-700 rounded-xl flex-shrink-0">
              ...
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full bg-[#1a1a1a] border border-gray-700 rounded-xl pl-9 pr-8 py-3 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-blue-500"
                />
                {searchQuery && (
                  <button onClick={() => setSearchQuery('')} className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5">
            ...
            <p className="text-[11px] text-gray-400 mb-2">{selectedCategory.name} • {filteredMovies.length} film</p>

            {isLoading && (
              <div className="flex justify-center py-16"><div className="w-8 h-8 border-3 border-blue-500/30 border-t-blue-500 rounded-full animate-spin" /></div>
            )}
            ...
            {!isLoading && !error && filteredMovies.length === 0 && (
              <div className="text-center py-16"><p className="text-gray-500 text-sm">Film bulunamadı</p></div>
            )}
              ...
              <div className="space-y-1">
                {filteredMovies.map((movie, index) => (
                  <motion.div
                    ...
                    className="flex items-center gap-3 p-3 bg-[#1a1a1a] rounded-xl cursor-pointer hover:bg-[#252525] border border-gray-800/30 active:scale-[0.99] transition-all"
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[10px] text-gray-500 truncate">{movie.group}</p>
                    </div>
